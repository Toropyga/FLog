...
## Настройка
Предварительная настройка параметров по умолчанию может осуществляться или непосредственно в самом классе, или с помощью именованных констант или передачей параметров при инициализации класса.
...
* LOG_CLEAR - вызывать ли очистку директории от старых файлов

При инициализации класса для настройки можно передать массив со следующими ключами:
* log_root_dir - путь к корневой директории
* log_path - путь к директории логов относительно корневой директории
* log_dir - имя директории логов
* log_name - имя файла логов
* log_max_size - максимальный размер файла логов в мегабайтах (Мб)
* log_time - количество дней на протяжении которых сохраняются логи
* log_level - уровень логов
* log_save_now - сохранять в лог сразу или по окончании работы
* log_system_info - объём служебной информации в логе ('full', 'advanced', 'simple')
* log_clear - вызывать ли очистку директории от старых файлов
